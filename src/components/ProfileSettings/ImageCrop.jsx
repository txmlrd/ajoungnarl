...
const ImageCrop = ({ file, onComplete, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      ...
      >
        {imgSrc && (
          <ReactCrop crop={crop} onChange={(_, percentCrop) => setCrop(percentCrop)} onComplete={(c) => setCompletedCrop(c)} aspect={1} className="max-w-full max-h-[60vh]">
            <img ref={imgRef} src={imgSrc} alt="Crop" onLoad={onImageLoad} className="max-w-full max-h-[60vh] object-contain" />
          </ReactCrop>
        )}

        <div className="flex gap-3 *:cursor-pointer *:transition-all">
          <button onClick={onClose} className="px-4 py-2 border-1 border-black text-black hover:bg-gray-200">
            Cancel
          </button>
          {completedCrop && (
            <button onClick={getCroppedImage} className="px-4 py-2 bg-black text-white border-1 border-black hover:bg-gray-500 hover:text-black">
              Crop & Save
            </button>
          )}
        </div>
